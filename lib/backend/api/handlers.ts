...
import { Subscription, Profile, AppConfig, SubscriptionUserInfo } from '../proxy/types';
...
export async function handleApiRequest(request: Request, env: Env): Promise<Response> {
    ...
    if (path === '/system/setup') {
        ...
        try {
            ...
            headers.append('Set-Cookie', `${COOKIE_NAME}=${token}; Path=/; HttpOnly${secureFlag}; SameSite=Strict; Max-Age=${SESSION_DURATION / 1000}`);
    ...
    switch (path) {
        ...
        case '/node_count': {
            ...
            const result: { count: number; userInfo: Partial<SubscriptionUserInfo> | null; nodes?: any[] } = { count: 0, userInfo: null };

            try {
                // 情况一：直接提供内容（文件导入/文本粘贴）
                if (rawContent) {
                    let nodes = parse(rawContent);
                    nodes = await process(nodes, {
                        exclude: exclude || '',
                        dedupe: false,
                ...
                if (responses[1].status === 'fulfilled' && responses[1].value.ok) {
                    ...
                    try {
                        ...
                        parsedNodes = await process(parsedNodes, {
                            dedupe: false,
