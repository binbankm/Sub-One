...
export type ContentFormat = 'clash' | 'sip008' | 'uri-list' | 'base64' | 'surge' | 'loon' | 'qx' | 'html' | 'unknown';
...
export function detectFormat(content: string): ContentFormat {
    ...
    if (isPlatformFormat(trimmed)) {
        if (/^.*=\s*(ss|shadowsocks|ssr|shadowsocksr|vmess|vless|trojan|http|https|snell|tuic|hysteria2|hy2|wireguard|wg),/i.test(trimmed)) {
    ...
    // 6. URI 列表检测
    // 逻辑：包含常见协议头
    if (/^(ss|ssr|vmess|vless|trojan|hysteria|hy2|tuic|wg|wireguard|socks|http|https|snell|anytls):\/\//mi.test(trimmed)) {
        return 'uri-list';
    }
...
function isLikelyBase64(str: string): boolean {
    ...
    try {
        ...
        // 解码后应该包含协议头
        return /^(ss|ssr|vmess|vless|trojan|hysteria|hy2|tuic|anytls|snell|wg|wireguard|socks|http|https):\/\//mi.test(decoded);
    } catch {
        return false;
    }
}
