...
export class SubscriptionConverter {
    ...
    // 以下方法已移到独立的转换器模块
    toClash(nodes: Node[], options: ConverterOptions = {}): string {
        return toClash(nodes, options);
    }

    toSingBox(nodes: Node[], options: ConverterOptions = {}): string {
        return toSingBox(nodes, options);
    }

    toSurge(nodes: Node[], options: ConverterOptions = {}): string {
        return toSurge(nodes, options);
    }

    toLoon(nodes: Node[], options: ConverterOptions = {}): string {
        return toLoon(nodes, options);
    }

    toQuantumultX(nodes: Node[], options: ConverterOptions = {}): string {
...
// 导出类型和选项
export type { ConverterOptions } from '../types';
