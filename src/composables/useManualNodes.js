...
import { COUNTRY_CODE_MAP, detectRegion, getRegionCode } from '../lib/utils.js';
...
export function useManualNodes(initialNodesRef, markDirty) {
  ...
  function autoSortNodes() {
    ...
    manualNodes.value.sort((a, b) => {
      const regionNameA = detectRegion(a.name);
      const regionNameB = detectRegion(b.name);

      const regionCodeA = getRegionCode(regionNameA) || 'zz';
      const regionCodeB = getRegionCode(regionNameB) || 'zz';

      const indexA = regionOrder.indexOf(regionCodeA);
      const indexB = regionOrder.indexOf(regionCodeB);
