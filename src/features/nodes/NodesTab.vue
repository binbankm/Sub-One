...
const props = defineProps<{
  ...
  addSubscriptionsFromBulk: (subs: Subscription[]) => void;
  deduplicateNodes: () => void;
  autoSortNodes: () => void;
...
const emit = defineEmits<{
  ...
  (e: 'node-deleted', id: string): void;
  (e: 'all-nodes-deleted'): void;
  (e: 'batch-nodes-deleted', ids: string[]): void;
}>();
...
const handleEditNode = (nodeId: string) => {
  const node = props.manualNodes.find(n => n.id === nodeId);
  if (node) {
...
const handleSaveNode = async (updatedNode?: Node) => {
  ...
  if (isNewNode.value) {
    props.addNode(nodeToSave);
  } else {
    props.updateNode(nodeToSave);
  }
  
  await props.saveData('节点');
  props.triggerUpdate();
...
const handleConfirmDeleteSingleNode = async () => {
  ...
  props.deleteNode(deletingItemId.value);
  emit('node-deleted', deletingItemId.value); // For cleanup in profiles
  
  await props.saveData('节点删除');
  props.triggerUpdate();
...
const handleDeleteAllNodes = async () => {
  ...
  emit('all-nodes-deleted');
  
  await props.saveData('节点清空');
  props.triggerUpdate();
...
const handleBatchDelete = async (ids: string[]) => {
  ...
  props.batchDeleteNodes(ids);
  emit('batch-nodes-deleted', ids);
  
  await props.saveData(`批量删除 ${ids.length} 个节点`);
  props.triggerUpdate();
  toggleBatchDeleteMode();
};
...
const handleBulkImport = async (importText: string) => {
  ...
  if (subs.length > 0) props.addSubscriptionsFromBulk(subs);
  if (nodes.length > 0) props.addNodesFromBulk(nodes);
  
  await props.saveData('批量导入');
  props.triggerUpdate();
...
const handleDeduplicate = async () => {
  props.deduplicateNodes();
  await props.saveData('节点去重');
  props.triggerUpdate();
};

const handleAutoSort = async () => {
  props.autoSortNodes();
  await props.saveData('节点排序');
  props.triggerUpdate();
};
