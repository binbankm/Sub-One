...
import { REGION_KEYWORDS, REGION_ORDER } from '../../utils/constants';
...
export function useManualNodes(initialNodesRef: Ref<Node[] | null>) {
   ...
   * 保存节点（新增或更新）
   */
  function saveNode(node: Node, isNew: boolean): { success: boolean; message?: string } {
    if (!node.url) return { success: false, message: '节点链接不能为空' };

    if (isNew) {
      addNode({ ...node, id: node.id || crypto.randomUUID() });
    } else {
      updateNode(node);
    }
    return { success: true };
  ...
  function batchDeleteNodes(ids: string[]) {
    ...
    const initialCount = manualNodes.value.length;
    manualNodes.value = manualNodes.value.filter(n => !ids.includes(n.id));

    if (paginatedManualNodes.value.length === 0 && manualNodesCurrentPage.value > 1) {
      manualNodesCurrentPage.value--;
    }
    return initialCount - manualNodes.value.length;
  }
