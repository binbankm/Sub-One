...
import type { Profile, Subscription, Node } from '../../../types/index';
...
const handleDeselectAll = (listName: 'subscriptions' | 'manualNodes', sourceArray: { id: string }[]) => {
  ...
  localProfile.value[listName] = (localProfile.value[listName] as string[]).filter(id => !sourceIds.includes(id));
};
          ...
          <div>
            ...
            <div class="flex gap-2">
              <input type="text" id="profile-custom-id" v-model="localProfile.customId"
                placeholder="如: home, game (限字母、数字、-、_)" class="input-modern-enhanced flex-1">
              <button @click="handleGenerateShortId" type="button" 
                class="px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors shadow-sm"
                title="生成随机短 ID">
            ...
            <p class="text-xs text-gray-400 mt-1.5">设置后，订阅链接会变为：/token/<span class="text-indigo-500 font-bold">{{ localProfile.customId || 'id' }}</span></p>
          </div>
