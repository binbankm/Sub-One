...
<template>
  <div
    class="group w-full card-modern p-3 sm:p-4 transition-all duration-300 hover:scale-[1.02] flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 border-b border-gray-200 dark:border-gray-700/50 last:border-0"
  ...
  >
    ...
    <div class="flex items-center gap-3 w-full">
      ...
      <div class="flex-shrink-0 w-auto whitespace-nowrap">
        <div
          class="text-xs font-bold px-2.5 py-1 rounded-full border inline-block"
      ...
      <!-- 移动端始终显示操作按钮 -->
      <div class="flex-shrink-0 flex items-center gap-1 md:hidden">
        <button @click.stop="emit('edit')" class="p-2 rounded-full hover:bg-gray-500/10 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" title="编辑节点">
        ...
        <button @click.stop="emit('delete')" class="p-2 rounded-full hover:bg-red-500/10 text-gray-400 hover:text-red-500" title="删除节点">
    ...
    <!-- 第二行：主机名和端口信息（移动端可见） -->
    <div class="flex items-center justify-between w-full text-xs text-gray-500 dark:text-gray-400 px-9">
      <div class="flex-1 min-w-0">
        <p class="font-mono truncate" :title="hostAndPort.host">
          {{ hostAndPort.host || 'N/A' }}
        </p>
      </div>
      <div class="flex-shrink-0 w-auto px-2 text-right">
        <p class="font-mono">
    ...
    <!-- 桌面端操作按钮 -->
    <div class="hidden md:flex flex-shrink-0 items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
