  ...
  >
    <div class="flex items-center justify-between gap-2 sm:gap-3">
      ...
      <div class="flex-shrink-0 flex items-center gap-1 sm:gap-2">
        <button @click.stop="emit('edit')" class="p-1.5 sm:p-2 rounded-lg hover:bg-indigo-500/20 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all duration-200 flex items-center gap-1 text-xs" title="编辑节点">
        ...
        <button @click.stop="emit('delete')" class="p-1.5 sm:p-2 rounded-lg hover:bg-red-500/20 text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-all duration-200 flex items-center gap-1 text-xs" title="删除节点">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 sm:h-4 sm:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
