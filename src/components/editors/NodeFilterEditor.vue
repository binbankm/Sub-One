...
const parseValue = (val: string) => {
    ...
    cleanLines.forEach(line => {
        if (line.startsWith('proto:')) {
            ...
            // 尝试直接匹配完整地区字符串（兼容旧逻辑或单一地区情况）
            let matchedWholeLine = false;
            for (const r of regions) {
                 if (line === `(${r.value})` || line === r.value) {
                     foundRegions.push(r.value);
                     matchedWholeLine = true;
                     break;
                 }
            }

            if (!matchedWholeLine) {
                // 处理复杂的组合规则 (HK|TW|SG|...) 或纯关键词
                const cleanKey = line.replace(/^\(/, '').replace(/\)$/, '');
                const tokens = cleanKey.split('|');
                
                tokens.forEach(token => {
                    const trimmedToken = token.trim();
                    if (!trimmedToken) return;

                    let isRegionPart = false;
                    // 检查此 token 是否归属于某个已知地区
                    for (const r of regions) {
                        const regionParts = r.value.split('|');
                        if (regionParts.includes(trimmedToken)) {
                            foundRegions.push(r.value);
                            isRegionPart = true;
                            break;
                        }
                    ...
                    if (!isRegionPart) {
                        foundKeywords.push(trimmedToken);
                    }
                });
            }
        }
