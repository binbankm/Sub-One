...
<template>
  ...
  <div v-else class="w-full container-optimized">
    ...
    <div class="space-y-6 lg:space-y-8">
      ...
      <!-- 订阅管理标签页 -->
      <div v-if="activeTab === 'subscriptions'" class="bg-white/60 dark:bg-gray-800/75 rounded-3xl p-8 border border-gray-300/50 dark:border-gray-700/30 shadow-xl hover:shadow-2xl transition-all duration-500">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 gap-6">
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-3xl flex items-center justify-center shadow-2xl transform hover:rotate-6 transition-transform duration-300">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" />
              </svg>
            </div>
            <div>
              <h2 class="text-2xl font-bold gradient-text-enhanced">订阅管理</h2>
              <p class="text-base text-gray-500 dark:text-gray-400">管理您的机场订阅</p>
            </div>
            <span class="px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-200 bg-gradient-to-r from-gray-200 to-gray-300 dark:from-gray-700/50 dark:to-gray-600/50 rounded-2xl shadow-lg">{{ subscriptions.length }}</span>
          </div>
          <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-start lg:justify-end">
            <div class="flex items-center gap-3 flex-shrink-0">
              <button @click="handleAddSubscription" class="btn-modern-enhanced btn-add text-sm font-semibold px-8 py-3 transform hover:scale-105 transition-all duration-300 rounded-2xl">新增</button>
              <button 
                @click="handleUpdateAllSubscriptions" 
                :disabled="isUpdatingAllSubs"
                class="btn-modern-enhanced btn-update text-sm font-semibold px-8 py-3 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transform hover:scale-105 transition-all duration-300 rounded-2xl"
              >
            ...
            <div class="flex items-center gap-3 flex-shrink-0">
              <button 
                ...
                :class="isSortingSubs ? 'btn-modern-enhanced btn-sort sorting text-sm font-semibold px-8 py-3 flex items-center gap-2 transform hover:scale-105 transition-all duration-300 rounded-2xl' : 'btn-modern-enhanced btn-sort text-sm font-semibold px-8 py-3 flex items-center gap-2 transform hover:scale-105 transition-all duration-300 rounded-2xl'"
              >
              ...
              <div class="relative" ref="subsMoreMenuRef">
                ...
                <Transition name="slide-fade-sm">
                  <div v-if="showSubsMoreMenu" class="absolute right-0 mt-3 w-40 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl z-10 ring-1 ring-black ring-opacity-5">
                    <button @click="showDeleteSubsModal = true; showSubsMoreMenu=false" class="w-full text-left px-5 py-3 text-sm text-red-500 hover:bg-red-500/10 rounded-xl">清空所有</button>
                  </div>
                </Transition>
              </div>
            </div>
        ...
        <div v-if="subscriptions.length > 0">
          <draggable 
            ...
            <template #item="{ element: subscription }">
              <div class="cursor-move transform hover:scale-105 transition-transform duration-300">
                  <Card 
                      :sub="subscription" 
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              <div v-for="subscription in paginatedSubscriptions" :key="subscription.id" class="transform hover:scale-105 transition-transform duration-300">
                  <Card 
                      :sub="subscription" 
                      @delete="handleDeleteSubscriptionWithCleanup(subscription.id)" 
                      @change="handleSubscriptionToggle(subscription)" 
          ...
          <div v-if="subsTotalPages > 1 && !isSortingSubs" class="flex justify-center items-center space-x-4 mt-10 text-sm font-medium">
        ...
        <div v-else class="text-center py-20 text-gray-500 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-3xl">
          ...
          <h3 class="text-2xl font-bold gradient-text-enhanced mb-3">没有订阅</h3>
          <p class="text-base text-gray-500 dark:text-gray-400">从添加你的第一个订阅开始。</p>
        </div>
      </div>

      <!-- 订阅组标签页 -->
      <div v-if="activeTab === 'profiles'" class="bg-white/60 dark:bg-gray-800/75 rounded-3xl p-8 border border-gray-300/50 dark:border-gray-700/30 shadow-xl hover:shadow-2xl transition-all duration-500">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 gap-6">
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 bg-gradient-to-br from-purple-500 to-pink-600 rounded-3xl flex items-center justify-center shadow-2xl transform hover:rotate-6 transition-transform duration-300">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
              </svg>
            </div>
            <div>
              <h2 class="text-2xl font-bold gradient-text-enhanced">订阅组</h2>
              <p class="text-base text-gray-500 dark:text-gray-400">组合管理节点</p>
            </div>
            <span class="px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-200 bg-gradient-to-r from-gray-200 to-gray-300 dark:from-gray-700/50 dark:to-gray-600/50 rounded-2xl shadow-lg">{{ profiles.length }}</span>
          </div>
          <div class="flex items-center gap-3 w-full lg:w-auto justify-start lg:justify-end">
            <button @click="handleAddProfile" class="btn-modern-enhanced btn-add text-sm font-semibold px-8 py-3 transform hover:scale-105 transition-all duration-300 rounded-2xl">新增</button>
            <div class="relative" ref="profilesMoreMenuRef">
              ...
              <Transition name="slide-fade-sm">
                <div v-if="showProfilesMoreMenu" class="absolute right-0 mt-3 w-36 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl z-10 ring-1 ring-black ring-opacity-5">
                  <button @click="showDeleteProfilesModal = true; showProfilesMoreMenu=false" class="w-full text-left px-5 py-3 text-sm text-red-500 hover:bg-red-500/10 rounded-xl">清空所有</button>
                </div>
              </Transition>
            </div>
          </div>
        ...
        <div v-if="profiles.length > 0" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          <div v-for="profile in paginatedProfiles" :key="profile.id" class="transform hover:scale-105 transition-transform duration-300">
            <ProfileCard
              :profile="profile"
        ...
        <div v-if="profilesTotalPages > 1" class="flex justify-center items-center space-x-4 mt-10 text-sm font-medium">
        ...
        <div v-if="profiles.length === 0" class="text-center py-20 text-gray-500 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-3xl">
          ...
          <h3 class="text-2xl font-bold gradient-text-enhanced mb-3">没有订阅组</h3>
          <p class="text-base text-gray-500 dark:text-gray-400">创建一个订阅组来组合你的节点吧！</p>
        </div>
      </div>

      <!-- 链接生成标签页 -->
      <div v-if="activeTab === 'generator'" class="bg-white/60 dark:bg-gray-800/75 rounded-3xl p-8 border border-gray-300/50 dark:border-gray-700/30 shadow-xl hover:shadow-2xl transition-all duration-500">
        <div class="flex items-center gap-4 mb-8">
          <div class="w-16 h-16 bg-gradient-to-br from-orange-500 to-red-600 rounded-3xl flex items-center justify-center shadow-2xl transform hover:rotate-6 transition-transform duration-300">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
            </svg>
          </div>
          <div>
            <h2 class="text-2xl font-bold gradient-text-enhanced">生成订阅链接</h2>
            <p class="text-base text-gray-500 dark:text-gray-400">生成和管理订阅链接</p>
          </div>
        </div>
      ...
      <!-- 手动节点标签页 -->
      <div v-if="activeTab === 'nodes'" class="bg-white/60 dark:bg-gray-800/75 rounded-3xl p-8 border border-gray-300/50 dark:border-gray-700/30 shadow-xl hover:shadow-2xl transition-all duration-500">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 gap-6">
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 bg-gradient-to-br from-green-500 to-emerald-600 rounded-3xl flex items-center justify-center shadow-2xl transform hover:rotate-6 transition-transform duration-300">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l-4 4-4-4M6 16l-4-4 4-4" />
              </svg>
            </div>
            <div>
              <h2 class="text-2xl font-bold gradient-text-enhanced">手动节点</h2>
              <p class="text-base text-gray-500 dark:text-gray-400">管理单个节点链接</p>
            </div>
            <span class="px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-200 bg-gradient-to-r from-gray-200 to-gray-300 dark:from-gray-700/50 dark:to-gray-600/50 rounded-2xl shadow-lg">{{ manualNodes.length }}</span>
          </div>
          <div class="flex flex-col lg:flex-row lg:items-center gap-4 w-full lg:w-auto justify-start lg:justify-end">
            <!-- 搜索框 -->
            <div class="relative w-full lg:w-56 flex-shrink-0">
              <input 
                type="text" 
                v-model="searchTerm"
                placeholder="搜索节点..."
                class="search-input-unified w-full text-sm px-5 py-3 rounded-2xl"
              />
              <svg class="absolute left-5 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex flex-wrap items-center gap-3">
              ...
              <!-- 主要操作按钮 -->
              <div class="flex items-center gap-3 flex-shrink-0">
                <button @click="handleAddNode" class="btn-modern-enhanced btn-add text-sm font-semibold px-8 py-3 transform hover:scale-105 transition-all duration-300 rounded-2xl">新增</button>
                
                <button @click="showBulkImportModal = true" class="btn-modern-enhanced btn-import text-sm font-semibold px-8 py-3 transform hover:scale-105 transition-all duration-300 rounded-2xl">批量导入</button>
              ...
              <div class="relative" ref="nodesMoreMenuRef">
                <button @click="showNodesMoreMenu = !showNodesMoreMenu" class="p-4 rounded-2xl hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors hover-lift">
                 ...
                 <Transition name="slide-fade-sm">
                  <div v-if="showNodesMoreMenu" class="absolute right-0 mt-3 w-40 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl z-10 ring-1 ring-black ring-opacity-5">
                    <button @click="showSubscriptionImportModal = true; showNodesMoreMenu=false" class="w-full text-left px-5 py-3 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl">导入订阅</button>
                    <button @click="handleAutoSortNodes(); showNodesMoreMenu=false" class="w-full text-left px-5 py-3 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl">一键排序</button>
                    <button @click="handleDeduplicateNodes" class="w-full text-left px-5 py-3 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl">一键去重</button>
                    <div class="border-t border-gray-200 dark:border-gray-700 my-2"></div>
                    <button @click="showDeleteNodesModal = true; showNodesMoreMenu=false" class="w-full text-left px-5 py-3 text-sm text-red-500 hover:bg-red-500/10 rounded-xl">清空所有</button>
                  </div>
                </Transition>
              </div>
            </div>
        ...
        <div v-if="manualNodes.length > 0">
          <div v-if="manualNodeViewMode === 'card'">
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-5">
              <div v-for="node in paginatedManualNodes" :key="node.id" class="transform hover:scale-105 transition-transform duration-300">
                <ManualNodeCard 
                  :node="node" 
                  @edit="handleEditNode(node.id)" 
                  @delete="handleDeleteNodeWithCleanup(node.id)" />
          ...
          <div v-if="manualNodesTotalPages > 1 && !isSortingNodes" class="flex justify-center items-center space-x-4 mt-10 text-sm font-medium">
        ...
        <div v-else class="text-center py-20 text-gray-500 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-3xl">
          ...
          <h3 class="text-2xl font-bold gradient-text-enhanced mb-3">没有手动节点</h3>
          <p class="text-base text-gray-500 dark:text-gray-400">添加分享链接或单个节点。</p>
        </div>
      </div>
    </div>
  </div>
