...
<template>
  ...
  <div v-else class="w-full container-optimized">
    ...
    <div class="space-y-6 lg:space-y-8">
      ...
      <!-- 订阅管理标签页 -->
      <div v-if="activeTab === 'subscriptions'" class="bg-white/60 dark:bg-gray-800/75 rounded-2xl p-6 border border-gray-300/50 dark:border-gray-700/30 shadow-lg hover:shadow-xl transition-all duration-300">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-xl font-bold gradient-text-enhanced">订阅管理</h2>
              <p class="text-sm text-gray-500 dark:text-gray-400">管理您的机场订阅</p>
            </div>
            <span class="px-3 py-1 text-sm font-semibold text-gray-700 dark:text-gray-200 bg-gray-200 dark:bg-gray-700/50 rounded-xl shadow-sm">{{ subscriptions.length }}</span>
          </div>
                      <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto justify-end sm:justify-start">
              <div class="flex items-center gap-2 flex-shrink-0">
                <button @click="handleAddSubscription" class="btn-modern-enhanced btn-add text-sm font-semibold px-6 py-2.5 transform hover:scale-105 transition-all duration-300">新增</button>
                <button 
                  @click="handleUpdateAllSubscriptions" 
                  :disabled="isUpdatingAllSubs"
                  class="btn-modern-enhanced btn-update text-sm font-semibold px-6 py-2.5 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transform hover:scale-105 transition-all duration-300"
                >
              ...
              <div class="flex items-center gap-2 flex-shrink-0">
                ...
                <div class="relative" ref="subsMoreMenuRef">
                  <button @click="showSubsMoreMenu = !showSubsMoreMenu" class="p-3 rounded-2xl hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors hover-lift">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600 dark:text-gray-300" viewBox="0 0 20 20" fill="currentColor"><path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a0 0 0 100-4 2 2 0 000 4z" /></svg>
                  </button>
                  <Transition name="slide-fade-sm">
                    <div v-if="showSubsMoreMenu" class="absolute right-0 mt-2 w-36 bg-white dark:bg-gray-800 rounded-2xl shadow-xl z-10 ring-1 ring-black ring-opacity-5">
        ...
        <div v-if="subscriptions.length > 0">
          ...
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5">
          ...
          <div v-if="subsTotalPages > 1 && !isSortingSubs" class="flex justify-center items-center space-x-4 mt-8 text-sm font-medium">
              <button @click="changeSubsPage(subsCurrentPage - 1)" :disabled="subsCurrentPage === 1" class="px-4 py-2 rounded-xl disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hover-lift">&laquo; 上一页</button>
              <span class="text-gray-500 dark:text-gray-400">第 {{ subsCurrentPage }} / {{ subsTotalPages }} 页</span>
              <button @click="changeSubsPage(subsCurrentPage + 1)" :disabled="subsCurrentPage === subsTotalPages" class="px-4 py-2 rounded-xl disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hover-lift">下一页 &raquo;</button>
          </div>
        </div>
        <div v-else class="text-center py-16 text-gray-500 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-3xl">
          <div class="w-20 h-20 mx-auto mb-6 rounded-3xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 flex items-center justify-center">
          ...
          <h3 class="text-xl font-bold gradient-text-enhanced mb-2">没有订阅</h3>
          <p class="text-sm text-gray-500 dark:text-gray-400">从添加你的第一个订阅开始。</p>
        </div>
      </div>

      <!-- 订阅组标签页 -->
      <div v-if="activeTab === 'profiles'" class="bg-white/60 dark:bg-gray-800/75 rounded-2xl p-6 border border-gray-300/50 dark:border-gray-700/30 shadow-lg hover:shadow-xl transition-all duration-300">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-xl font-bold gradient-text-enhanced">订阅组</h2>
              <p class="text-sm text-gray-500 dark:text-gray-400">组合管理节点</p>
            </div>
            <span class="px-3 py-1 text-sm font-semibold text-gray-700 dark:text-gray-200 bg-gray-200 dark:bg-gray-700/50 rounded-xl shadow-sm">{{ profiles.length }}</span>
          </div>
                      <div class="flex items-center gap-2 w-full sm:w-auto justify-end sm:justify-start">
              <button @click="handleAddProfile" class="btn-modern-enhanced btn-add text-sm font-semibold px-6 py-2.5 transform hover:scale-105 transition-all duration-300">新增</button>
            <div class="relative" ref="profilesMoreMenuRef">
              <button @click="showProfilesMoreMenu = !showProfilesMoreMenu" class="p-3 rounded-2xl hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors hover-lift">
              ...
              <Transition name="slide-fade-sm">
                <div v-if="showProfilesMoreMenu" class="absolute right-0 mt-2 w-32 bg-white dark:bg-gray-800 rounded-2xl shadow-xl z-10 ring-1 ring-black ring-opacity-5">
                  <button @click="showDeleteProfilesModal = true; showProfilesMoreMenu=false" class="w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-red-500/10">清空所有</button>
                </div>
              </Transition>
            </div>
          </div>
        ...
        <div v-if="profiles.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5">
          <ProfileCard
        ...
        <div v-if="profilesTotalPages > 1" class="flex justify-center items-center space-x-4 mt-8 text-sm font-medium">
          <button @click="changeProfilesPage(profilesCurrentPage - 1)" :disabled="profilesCurrentPage === 1" class="px-4 py-2 rounded-xl disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hover-lift">&laquo; 上一页</button>
          <span class="text-gray-500 dark:text-gray-400">第 {{ profilesCurrentPage }} / {{ profilesTotalPages }} 页</span>
          <button @click="changeProfilesPage(profilesCurrentPage + 1)" :disabled="profilesCurrentPage === profilesTotalPages" class="px-4 py-2 rounded-xl disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hover-lift">下一页 &raquo;</button>
        </div>
        <div v-if="profiles.length === 0" class="text-center py-16 text-gray-500 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-3xl">
          <div class="w-20 h-20 mx-auto mb-6 rounded-3xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
          </div>
          <h3 class="text-xl font-bold gradient-text-enhanced mb-2">没有订阅组</h3>
          <p class="text-sm text-gray-500 dark:text-gray-400">创建一个订阅组来组合你的节点吧！</p>
        </div>
      </div>

      <!-- 链接生成标签页 -->
      <div v-if="activeTab === 'generator'" class="bg-white/60 dark:bg-gray-800/75 rounded-2xl p-6 border border-gray-300/50 dark:border-gray-700/30 shadow-lg hover:shadow-xl transition-all duration-300">
        <div class="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 class="text-xl font-bold gradient-text-enhanced">生成订阅链接</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">生成和管理订阅链接</p>
          </div>
        </div>
      ...
      <!-- 手动节点标签页 -->
      <div v-if="activeTab === 'nodes'" class="bg-white/60 dark:bg-gray-800/75 rounded-2xl p-6 border border-gray-300/50 dark:border-gray-700/30 shadow-lg hover:shadow-xl transition-all duration-300">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-xl font-bold gradient-text-enhanced">手动节点</h2>
              <p class="text-sm text-gray-500 dark:text-gray-400">管理单个节点链接</p>
            </div>
        ...
        <div v-if="manualNodes.length > 0">
          <div v-if="manualNodeViewMode === 'card'">
             <draggable 
              v-if="isSortingNodes"
              tag="div" 
              class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4" 
              v-model="manualNodes" 
              :item-key="item => item.id" 
              animation="300" 
              :delay="200"
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
          ...
          <div v-if="manualNodesTotalPages > 1 && !isSortingNodes" class="flex justify-center items-center space-x-4 mt-8 text-sm font-medium">
            <button @click="changeManualNodesPage(manualNodesCurrentPage - 1)" :disabled="manualNodesCurrentPage === 1" class="px-4 py-2 rounded-xl disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hover-lift">&laquo; 上一页</button>
        ...
        <div v-else class="text-center py-16 text-gray-500 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-3xl">
          <div class="w-20 h-20 mx-auto mb-6 rounded-3xl bg-gradient-to-br from-green-500/20 to-emerald-500/20 flex items-center justify-center">
          ...
          <h3 class="text-xl font-bold gradient-text-enhanced mb-2">没有手动节点</h3>
          <p class="text-sm text-gray-500 dark:text-gray-400">添加分享链接或单个节点。</p>
        </div>
      </div>
    </div>
  </div>
