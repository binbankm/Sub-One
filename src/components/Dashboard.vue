    ...
    <div class="space-y-6 lg:space-y-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div class="bg-gradient-to-br from-white/80 to-white/40 dark:from-gray-800/90 dark:to-gray-800/60 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50 shadow-xl hover:shadow-2xl transition-all duration-300">
            <div class="flex items-center justify-between mb-4">
              ...
              <div class="w-12 h-12 rounded-xl bg-green-100 dark:bg-green-900/50 flex items-center justify-center text-green-600 dark:text-green-400">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                </svg>
              </div>
      ...
      <div v-if="activeTab === 'subscriptions'" class="bg-white/60 dark:bg-gray-800/75 rounded-2xl p-8 lg:p-10 border border-gray-300/50 dark:border-gray-700/30 shadow-lg hover:shadow-xl transition-all duration-300">
        ...
        <div v-if="subscriptions.length > 0">
          ...
          <div v-if="subsTotalPages > 1 && !isSortingSubs" class="flex justify-center items-center gap-1.5 sm:gap-4 mt-10 text-base font-medium px-2">
              <button @click="changeSubsPage(subsCurrentPage - 1)" :disabled="subsCurrentPage === 1" class="min-w-[50px] sm:min-w-[100px] px-2 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-2xl disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hover-lift font-medium text-sm sm:text-base flex items-center justify-center">&laquo;<span class="hidden sm:inline ml-1">上一页</span></button>
        <span class="min-w-[65px] sm:min-w-[100px] text-center text-xs sm:text-sm text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap px-1">第{{ subsCurrentPage }}/{{ subsTotalPages }}页</span>
        <button @click="changeSubsPage(subsCurrentPage + 1)" :disabled="subsCurrentPage === subsTotalPages" class="min-w-[50px] sm:min-w-[100px] px-2 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-2xl disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hover-lift font-medium text-sm sm:text-base flex items-center justify-center"><span class="hidden sm:inline mr-1">下一页</span>&raquo;</button>
          </div>
        </div>
      ...
      <div v-if="activeTab === 'profiles'" class="bg-white/60 dark:bg-gray-800/75 rounded-2xl p-8 lg:p-10 border border-gray-300/50 dark:border-gray-700/30 shadow-lg hover:shadow-xl transition-all duration-300">
        ...
        <div v-if="profilesTotalPages > 1" class="flex justify-center items-center gap-1.5 sm:gap-4 mt-10 text-base font-medium px-2">
          <button @click="changeProfilesPage(profilesCurrentPage - 1)" :disabled="profilesCurrentPage === 1" class="min-w-[50px] sm:min-w-[100px] px-2 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-2xl disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hover-lift font-medium text-sm sm:text-base flex items-center justify-center">&laquo;<span class="hidden sm:inline ml-1">上一页</span></button>
        <span class="min-w-[65px] sm:min-w-[100px] text-center text-xs sm:text-sm text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap px-1">第{{ profilesCurrentPage }}/{{ profilesTotalPages }}页</span>
        <button @click="changeProfilesPage(profilesCurrentPage + 1)" :disabled="profilesCurrentPage === profilesTotalPages" class="min-w-[50px] sm:min-w-[100px] px-2 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-2xl disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hover-lift font-medium text-sm sm:text-base flex items-center justify-center"><span class="hidden sm:inline mr-1">下一页</span>&raquo;</button>
        </div>
      ...
      <div v-if="activeTab === 'nodes'" class="bg-white/60 dark:bg-gray-800/75 rounded-2xl p-8 lg:p-10 border border-gray-300/50 dark:border-gray-700/30 shadow-lg hover:shadow-xl transition-all duration-300">
        ...
        <div v-if="manualNodes.length > 0">
          ...
          <div v-if="manualNodesTotalPages > 1 && !isSortingNodes" class="flex justify-center items-center gap-1.5 sm:gap-4 mt-10 text-base font-medium px-2">
            <button @click="changeManualNodesPage(manualNodesCurrentPage - 1)" :disabled="manualNodesCurrentPage === 1" class="min-w-[50px] sm:min-w-[100px] px-2 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-2xl disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hover-lift font-medium text-sm sm:text-base flex items-center justify-center">&laquo;<span class="hidden sm:inline ml-1">上一页</span></button>
        <span class="min-w-[65px] sm:min-w-[100px] text-center text-xs sm:text-sm text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap px-1">第{{ manualNodesCurrentPage }}/{{ manualNodesTotalPages }}页</span>
