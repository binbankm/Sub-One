...
<template>
  <div
    class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-300 dark:border-gray-700 p-3 sm:p-5 group relative overflow-hidden transition-all duration-500 hover:scale-[1.02] flex flex-col shadow-sm hover:shadow-lg min-h-[150px] sm:min-h-[140px]"
  ...
  >
    ...
    <div class="relative z-10 flex items-start justify-between gap-2 sm:gap-3">
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-1.5 sm:gap-2 mb-2">
          ...
          <p class="font-bold text-base sm:text-lg text-gray-800 dark:text-gray-100 truncate" :title="profile.name">
          {{ profile.name }}
        </p>
        </div>
        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 break-words leading-relaxed">
      ...
      <div class="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300">
        <button @click.stop="emit('edit')" class="p-2.5 rounded-lg hover:bg-indigo-500/10 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all duration-200 hover-lift" title="编辑">
        ...
        <button @click.stop="emit('delete')" class="p-2.5 rounded-lg hover:bg-red-500/10 text-gray-400 hover:text-red-500 transition-all duration-200 hover-lift" title="删除">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
