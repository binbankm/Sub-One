...
<template>
  <div 
    class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-300 dark:border-gray-700 p-3 sm:p-4 group relative overflow-hidden transition-all duration-300 hover:scale-[1.02] flex items-center justify-between gap-2 sm:gap-3 shadow-sm hover:shadow-lg min-h-[80px] sm:min-h-[72px]"
  ...
  >
    ...
    <div class="relative z-10 flex items-center gap-2 sm:gap-3 overflow-hidden flex-1 min-w-0">
    ...
    <div class="relative z-10 flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300">
        <button @click.stop="emit('edit')" class="p-2.5 rounded-lg hover:bg-indigo-500/10 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all duration-200" title="编辑节点">
        ...
        <button @click.stop="emit('delete')" class="p-2.5 rounded-lg hover:bg-red-500/10 text-gray-400 hover:text-red-500 transition-all duration-200" title="删除节点">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
