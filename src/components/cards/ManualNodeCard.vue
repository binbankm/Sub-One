...
<template>
  <div 
    class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-300 dark:border-gray-700 p-3 sm:p-4 group relative overflow-hidden transition-all duration-300 hover:scale-[1.02] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm hover:shadow-lg"
  ...
  >
    ...
    <div class="relative z-10 flex items-center gap-3 overflow-hidden flex-1 min-w-0 w-full sm:w-auto">
      ...
      <div class="flex-1 min-w-0">
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 truncate break-all" :title="node.url">
          {{ node.url.substring(0, 50) }}{{ node.url.length > 50 ? '...' : '' }}
        </p>
      </div>
    </div>

    <div class="relative z-10 flex-shrink-0 flex items-center gap-1 sm:opacity-0 group-hover:opacity-100 transition-all duration-300 ml-auto sm:ml-0">
