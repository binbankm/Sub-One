...
// 计算属性
const activeSubscriptions = computed(() => props.subscriptions.filter(sub => sub.enabled).length);
const activeProfiles = computed(() => props.profiles.filter(profile => profile.enabled).length);
const activeManualNodes = computed(() => props.manualNodes.filter(node => node.enabled).length);
const totalNodeCount = computed(() => {
  let count = props.manualNodes.length;
...
const activeNodeCount = computed(() => {
  let count = props.manualNodes.filter(node => node.enabled).length;
  props.subscriptions.forEach(sub => {
